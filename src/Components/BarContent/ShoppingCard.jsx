...
const ShoppingCard = () => {
  const { isOpen, handleClose } = useCardProducts();
  const { cart, clearCart, itemAmount, total } = useCardProducts();

  return (
    <div
      className={`${
        isOpen ? "right-0" : "-right-full"
      } "  w-full bg-white fixed top-0 h-full shadow-2xl md:w-[35vw] lg:w-[40vw] xl:max-w-[30vw] transition-all duration-300 z-10 px-4 lg:px-[35px]"`}
    >
